...
class RequestTemplateCreator extends AbstractSchemaCreator <RequestTemplateCreatorContext> {
    ...
    void createPart(part, RequestTemplateCreatorContext ctx){
        ...
        if (part.type instanceof ComplexType || part.type instanceof SimpleType) {
            // 注释掉，以支持复杂对象 原问题参考 https://github.com/membrane/soa-model/issues/268 // part.type.create(this, ctx)
            ctx.element = part.type
            def attrs = [:]
            declNSifNeeded(getNSPrefix(ctx.element, ctx), part.type.namespaceUri, attrs, ctx)
            attrs.putAll(createAttributes(part.type, ctx))
            builder."${part.name}"(attrs) {
                part.type.model?.create(this, ctx)
                part.type.anyAttribute?.create(this, ctx)
            }
            return
        }
